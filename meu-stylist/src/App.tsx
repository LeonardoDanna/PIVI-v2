...
const colorPalettes: Record<string, { label: string; hex: string; season: string; description: string; colors: string[]; styles: string[] }> = {
  very_light: {
    ...
    hex: '#FAE7D0',
    season: 'Inverno Frio',
    description: 'Pele com subtom frio. Cores puras, gélidas e contrastantes realçam sua beleza natural.',
    colors: ['bg-blue-500', 'bg-pink-600', 'bg-emerald-500', 'bg-purple-700', 'bg-gray-900', 'bg-slate-200'],
    styles: ['Minimalista', 'Dramático', 'Alfaiataria', 'Gótico Suave', 'Moderno', 'Futurista']
  },
  light: {
    label: 'Clara',
    hex: '#E3C1A0',
    season: 'Verão Suave',
    description: 'Contraste delicado. Tons pastéis, lavanda e azul bebê harmonizam perfeitamente.',
    colors: ['bg-sky-200', 'bg-rose-300', 'bg-indigo-300', 'bg-teal-200', 'bg-purple-300', 'bg-slate-400'],
    styles: ['Romântico', 'Lady Like', 'Provençal', 'Clássico', 'Vintage', 'Cottagecore']
  },
  medium: {
    label: 'Média',
    hex: '#CFA880',
    season: 'Primavera Quente',
    description: 'Pele dourada e vibrante. Cores alegres como coral, turquesa e dourado são ideais.',
    colors: ['bg-orange-400', 'bg-lime-500', 'bg-yellow-400', 'bg-cyan-500', 'bg-red-500', 'bg-amber-300'],
    styles: ['Criativo', 'Tropical', 'Esportivo', 'Color Block', 'Casual Chic', 'Preppy']
  },
  tan: {
    label: 'Bronzeada',
    hex: '#A67B51',
    season: 'Outono Profundo',
    description: 'Tons terrosos, mostarda e verde militar complementam seu bronzeado natural.',
    colors: ['bg-orange-800', 'bg-green-800', 'bg-yellow-700', 'bg-red-900', 'bg-stone-700', 'bg-amber-700'],
  ...
  dark: {
    ...
    description: 'Pele rica e quente. Tons de especiarias, vinhos profundos e azul marinho ficam elegantes.',
    colors: ['bg-rose-900', 'bg-blue-900', 'bg-emerald-900', 'bg-yellow-600', 'bg-purple-900', 'bg-orange-900'],
    styles: ['Sofisticado', 'Glamour', 'Urbano', 'Executivo', 'Retrô', 'Barroco']
  },
  deep: {
    label: 'Retinta',
    hex: '#422618',
    season: 'Inverno Brilhante',
    description: 'Alto contraste e profundidade. Cores neon, branco puro e cores primárias vibrantes.',
    colors: ['bg-fuchsia-600', 'bg-blue-600', 'bg-yellow-300', 'bg-white', 'bg-red-600', 'bg-violet-600'],
    styles: ['Streetwear', 'High Fashion', 'Vibrante', 'Geométrico', 'Esportivo Deluxe', 'Pop Art']
  }
};

const tabs = [
  { id: 'today', label: 'Visão Geral', icon: <Sun size={20} />, subtitle: 'Sua dose diária de estilo' },
  { id: 'closet', label: 'Armário Virtual', icon: <Package size={20} />, subtitle: 'Monte looks completos' }, // NOME CORRIGIDO
...
const PersonalStylistApp = () => {
  ...
  const [tryOnState, setTryOnState] = useState({
    ...
    clothImage: null as string | null,
    isGenerating: false,
  ...
  // Estado para feedback de "Salvar Look"
  ...
  // Funções Provador Virtual
  const handleUploadUser = () => setTryOnState(prev => ({ ...prev, userImage: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=800&auto=format&fit=crop' }));
  const handleUploadCloth = () => setTryOnState(prev => ({ ...prev, clothImage: 'https://images.unsplash.com/photo-1596755094514-f87e34085b2c?q=80&w=800&auto=format&fit=crop' }));
  const handleGenerate = () => {
    setTryOnState(prev => ({ ...prev, isGenerating: true }));
    setTimeout(() => {
        setTryOnState(prev => ({ ...prev, isGenerating: false, resultImage: 'https://images.unsplash.com/photo-1605908502724-9093a79a1b39?q=80&w=800&auto=format&fit=crop' }));
    }, 3000);
  };
  const resetTryOn = () => setTryOnState({ userImage: null, clothImage: null, isGenerating: false, resultImage: null });

  // Função Salvar Look (Armário Virtual)
  const handleSaveOutfit = () => {
    if (isOutfitSaved) return;
    setIsSavingOutfit(true);
    // Simula delay de salvar no banco de dados
    setTimeout(() => {
      setIsSavingOutfit(false);
      setIsOutfitSaved(true);
      // Reseta o estado depois de 2 segundos para permitir salvar outro
                ...
                <div className="animate-fade-in h-[calc(100vh-140px)]">
                    ...
                    <div className="grid grid-cols-12 gap-8 h-[90%]">
                        <div className="col-span-5 flex flex-col gap-6">
                            <div onClick={handleUploadUser} className={`flex-1 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden group ${tryOnState.userImage ? 'border-purple-500 bg-purple-50' : 'border-slate-300 hover:border-slate-400 hover:bg-slate-50'}`}>
                                {tryOnState.userImage ? (<><img src={tryOnState.userImage} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100 transition" /><div className="absolute top-3 right-3 bg-white p-1 rounded-full shadow-sm"><Check className="text-green-600" size={16}/></div></>) : (<div className="text-center p-6"><div className="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center mx-auto mb-3"><User className="text-slate-400" /></div><p className="font-bold text-slate-700">1. Seu Corpo</p><p className="text-xs text-slate-400 mt-1">Clique para fazer upload</p></div>)}
                            </div>
                            <div onClick={handleUploadCloth} className={`flex-1 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden group ${tryOnState.clothImage ? 'border-purple-500 bg-purple-50' : 'border-slate-300 hover:border-slate-400 hover:bg-slate-50'}`}>
                                {tryOnState.clothImage ? (<><img src={tryOnState.clothImage} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100 transition" /><div className="absolute top-3 right-3 bg-white p-1 rounded-full shadow-sm"><Check className="text-green-600" size={16}/></div></>) : (<div className="text-center p-6"><div className="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center mx-auto mb-3"><Shirt className="text-slate-400" /></div><p className="font-bold text-slate-700">2. A Peça</p><p className="text-xs text-slate-400 mt-1">Clique para escolher a roupa</p></div>)}
                            </div>
                            <button onClick={handleGenerate} disabled={!tryOnState.userImage || !tryOnState.clothImage || tryOnState.isGenerating} className={`w-full py-4 rounded-xl font-bold text-white flex items-center justify-center gap-2 transition-all ${(!tryOnState.userImage || !tryOnState.clothImage) ? 'bg-slate-300 cursor-not-allowed' : 'bg-slate-900 hover:bg-purple-600 shadow-lg hover:shadow-purple-200'}`}>
                                {tryOnState.isGenerating ? (<><RefreshCw className="animate-spin" size={20} /> Processando IA...</>) : (<><Sparkles size={20} /> Gerar Provador</>)}
                            </button>
                        </div>
                        <div className="col-span-7 bg-slate-100 rounded-3xl relative overflow-hidden border border-slate-200 flex items-center justify-center">
                            {tryOnState.resultImage ? (<div className="relative w-full h-full group"><img src={tryOnState.resultImage} className="w-full h-full object-cover animate-fade-in" /><div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex justify-center gap-4"><button className="bg-white/20 backdrop-blur text-white p-3 rounded-full hover:bg-white/40 transition"><Share2 size={20}/></button><button className="bg-white text-black px-6 py-3 rounded-full font-bold hover:scale-105 transition flex items-center gap-2"><Download size={18}/> Salvar Look</button></div></div>) : (<div className="text-center p-8 max-w-xs"><div className="w-20 h-20 bg-white rounded-full shadow-sm flex items-center justify-center mx-auto mb-4 animate-pulse"><Sparkles className="text-purple-300" size={32} /></div><h4 className="font-bold text-slate-400 text-lg">Aguardando Uploads</h4><p className="text-sm text-slate-400 mt-2">Carregue sua foto e a foto da roupa para ver a mágica acontecer.</p></div>)}
                        </div>
                    </div>
                </div>
            )}
